  ...
  return (
    <article className="client-record">
      ...
      <form onSubmit={form.handleSubmit((values) => update.mutate(values))} noValidate>
        ...
        <div className="row-actions"><button className="secondary" type="button" onClick={() => runAction(() => setEditing(false))}>Cancel</button><button className="primary" disabled={update.isPending} aria-busy={update.isPending}>{update.isPending ? "Saving changes…" : "Save changes"}</button></div>
      </form></DialogSurface>
    </article>
  );
}
...
export function OwnerWorkspace({ workspace, section = "clients", onBack, refreshWork }: Readonly<{ workspace: WorkspaceGroup; section?: AdminSection; onBack: () => void; refreshWork: () => void }>) {
  ...
  return (
    <section className="admin-layout">
      ...
      {section === "invitations" && <div className="admin-grid"><section className="panel span"><div className="panel-heading"><div><p className="eyebrow">Access requests</p><h2>Invitations</h2></div><button className="primary" onClick={() => setCreatePanel("invite")}>Invite someone</button></div>{access.isPending && <LoadingBlock label="Loading invitations" />}<ErrorNote error={access.error || quickAction.error} />{data?.invitations.map((item) => { const canReissue = item.status === "pending" || (item.status !== "accepted" || !activeClientAccess(item)); const revokePath = `/workspaces/${workspace.id}/invitations/${item.id}/revoke`; const reissuePending = quickAction.isPending && quickAction.variables?.path === `/workspaces/${workspace.id}/invitations`; const revokePending = quickAction.isPending && quickAction.variables?.path === revokePath; return <div className="access-row" key={item.id}><div><strong>{item.email}</strong><p>{item.role.replaceAll("-", " ")}{item.projectId ? ` · ${projectName(item.projectId)}` : ""} · {item.status} · delivery {item.deliveryStatus}</p></div><div className="row-actions">{item.status === "pending" && <button className="danger compact" disabled={quickAction.isPending} aria-busy={revokePending} onClick={() => quickAction.mutate({ path: revokePath, method: "POST" })}>{revokePending ? "Revoking…" : "Revoke"}</button>}{canReissue && <button className="secondary" disabled={quickAction.isPending} aria-busy={reissuePending} onClick={() => quickAction.mutate({ path: `/workspaces/${workspace.id}/invitations`, method: "POST", body: invitePayload(item) })}>{reissuePending ? "Issuing…" : item.status === "pending" ? "Replace" : "Invite again"}</button>}</div></div>; })}{data && !data.invitations.length && <p className="empty-copy">No invitations have been issued.</p>}</section></div>}
      {createPanel === "client" && <DialogSurface title="Add a client" onClose={() => setCreatePanel(undefined)}>
            <form onSubmit={clientForm.handleSubmit((values) => clientMutation.mutate(values))} noValidate>
              ...
              <div className="dialog-actions"><button type="button" className="secondary" onClick={() => runAction(() => setCreatePanel(undefined))}>Cancel</button><button className="primary" disabled={clientMutation.isPending} aria-busy={clientMutation.isPending}>{clientMutation.isPending ? "Saving client…" : "Save client"}</button></div><ErrorNote error={clientMutation.error} />
            </form>
      </DialogSurface>}
      {createPanel === "project" && <DialogSurface title="Create a project" onClose={() => setCreatePanel(undefined)}>
            <form onSubmit={projectForm.handleSubmit((values) => projectMutation.mutate(values))} noValidate>
              ...
              <div className="dialog-actions"><button type="button" className="secondary" onClick={() => runAction(() => setCreatePanel(undefined))}>Cancel</button><button className="primary" disabled={projectMutation.isPending} aria-busy={projectMutation.isPending}>{projectMutation.isPending ? "Creating project…" : "Create project"}</button></div><ErrorNote error={projectMutation.error} />
            </form>
      </DialogSurface>}
      {createPanel === "invite" && <DialogSurface title="Invite someone" onClose={() => setCreatePanel(undefined)}>
            <form onSubmit={inviteForm.handleSubmit((values) => inviteMutation.mutate(values))} noValidate>
              ...
              <div className="dialog-actions"><button type="button" className="secondary" onClick={() => runAction(() => setCreatePanel(undefined))}>Cancel</button><button className="primary" disabled={inviteMutation.isPending} aria-busy={inviteMutation.isPending}>{inviteMutation.isPending && <span className="button-spinner" aria-hidden="true" />}{inviteMutation.isPending ? "Issuing invitation…" : "Issue invitation"}</button></div><ErrorNote error={inviteMutation.error} />
            </form>
      </DialogSurface>}
      {createPanel === "assign" && <DialogSurface title="Assign a service-team member" onClose={() => setCreatePanel(undefined)}>
            <form onSubmit={assignmentForm.handleSubmit((values) => assignmentMutation.mutate(values))}>
              ...
              <div className="dialog-actions"><button type="button" className="secondary" onClick={() => runAction(() => setCreatePanel(undefined))}>Cancel</button><button className="primary" disabled={assignmentMutation.isPending} aria-busy={assignmentMutation.isPending}>{assignmentMutation.isPending ? "Assigning project…" : "Assign project"}</button></div><ErrorNote error={assignmentMutation.error} />
            </form>
      </DialogSurface>}
