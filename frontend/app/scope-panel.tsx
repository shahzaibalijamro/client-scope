...
function DraftEditor({ projectId, draft, canSubmit, reload }: Readonly<{
  ...
  return <section className="panel scope-editor" aria-labelledby="draft-heading">
    ...
    <AiRequirementReviewPanel projectId={projectId} draft={{ ...draft, revisionToken, groups, requirements }} reloadDraft={loadLatest} />
...
function VersionCard({ projectId, version, active, scope }: Readonly<{ projectId: string; version: ScopeVersion; active: boolean; scope: ScopeData }>) {
  ...
  return <article className={`scope-version ${active ? "current" : ""}`}>
    ...
    {version.terminal && <div className="terminal-result"><strong>{version.status === "approved" || version.status === "superseded" ? "Approved" : version.status === "withdrawn" ? "Withdrawn" : "Changes requested"} by {version.terminal.actor.displayName}</strong><p className="fine"><time dateTime={version.terminal.at}>{new Date(version.terminal.at).toLocaleString()}</time></p>{version.terminal.note && <p className="plain-text">{version.terminal.note}</p>}</div>}
    {version.supersession && <p className="notice">Superseded through formal change control on <time dateTime={version.supersession.at}>{new Date(version.supersession.at).toLocaleString()}</time>. The original approval and content remain preserved.</p>}
    {version.provenance && <p className="notice success">Established from an approved change proposal submitted by {version.provenance.proposalSubmitter.displayName} and approved by {version.provenance.approvingClient.displayName}.</p>}
