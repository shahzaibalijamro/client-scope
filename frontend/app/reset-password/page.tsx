...
function ResetPasswordContent() {
  ...
  const reset = useMutation({ mutationFn: (values: z.infer<typeof schema>) => api("/auth/reset-password", json("POST", { token, ...values }), z.object({ message: z.string(), csrfToken: z.string() }).strict()) });
  return <main className="center-layout"><section className="focus-card"><p className="eyebrow">Account recovery</p><h1>Choose a new password</h1><p>Use 12–128 characters. Spaces and Unicode are welcome; the value is never trimmed.</p>{!token ? <><p className="notice error" role="alert">This reset link is unavailable.</p><Link className="secondary button-link" href="/forgot-password">Request another reset link</Link></> : reset.isSuccess ? <><p className="notice success" role="status">{reset.data.message}</p><Link className="primary button-link" href="/sign-in">Return to sign in</Link></> : <form onSubmit={form.handleSubmit((values) => reset.mutate(values))}><label>New password<input type="password" autoComplete="new-password" {...form.register("password")} />{form.formState.errors.password && <small role="alert">{form.formState.errors.password.message}</small>}</label><label>Confirm new password<input type="password" autoComplete="new-password" {...form.register("confirmation")} />{form.formState.errors.confirmation && <small role="alert">{form.formState.errors.confirmation.message}</small>}</label><button className="primary" disabled={reset.isPending} aria-busy={reset.isPending}>{reset.isPending ? "Changing password…" : "Change password"}</button></form>}{reset.error && <><p className="notice error" role="alert">{reset.error.message}</p><Link className="secondary button-link" href="/forgot-password">Request another reset link</Link></>}</section></main>;
}
