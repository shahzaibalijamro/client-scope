...
export function AppShell({ user, work, onLogout, children, demoMode: requestedDemoMode = false }: { user: User; work?: WorkResponse; onLogout: () => void; children: ReactNode; demoMode?: boolean }) {
  ...
  const navigation = <><div className="shell-brand" title="ClientScope"><span className="brand-mark">C</span><strong>Client<span>Scope</span></strong></div><ProjectSwitcher work={work} /><nav aria-label="Primary navigation"><p className="nav-label">Navigate</p><NavButton href="/work" active={route.kind === "work"} icon={<BriefcaseBusiness size={19} />} label="My Work" onNavigate={() => setMobileOpen(false)}>My Work</NavButton>{Boolean(work?.invitations.length) && <NavButton href="/work?view=invitations" active={false} icon={<Bell size={19} />} label="Invitations" onNavigate={() => setMobileOpen(false)}>Invitations <span className="nav-count">{work!.invitations.length}</span></NavButton>}<p className="nav-label">Workspace Admin</p>{ownerWorkspaces.map((workspace) => <NavButton key={workspace.id} href={adminHref(workspace.id)} active={route.kind === "admin" && route.workspaceId === workspace.id} icon={<Building2 size={19} />} label={workspace.name} onNavigate={() => setMobileOpen(false)}>{workspace.name}</NavButton>)}</nav><button className="collapse-button" title={collapsed ? "Expand sidebar" : "Collapse sidebar"} onClick={toggleCollapsed} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}>{collapsed ? <ChevronRight size={18} /> : <><ChevronLeft size={18} /><span>Collapse sidebar</span></>}</button></>;
