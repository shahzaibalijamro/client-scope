...
test("an authorized member downloads the shared record while another account receives safe cross-project denial", async ({ browser }) => {
  ...
  await owner.page.goto("/"); await owner.page.getByRole("button", { name: new RegExp(projectName) }).click(); await expect(owner.page.getByRole("heading", { name: "Project export" })).toBeVisible();
