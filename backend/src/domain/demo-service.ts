...
export class DemoService {
  ...
  private canonicalDocuments(tenantId: mongoose.Types.ObjectId, ownerId: mongoose.Types.ObjectId, approverId: mongoose.Types.ObjectId, now: Date) {
    ...
    const activityDocs = projects.flatMap((projectId, index) => [
      { _id: id(`activity:${index}:scope`), workspaceId: tenantId, projectId, actorId: approverId, actorName: "Jordan Lee", action: "scope.version-approved", audience: "project", context: { actorRole: role, versionId: String(scopeIds[index]), versionNumber: index < 2 ? 2 : 1, outcome: "approved" }, occurredAt: at(now, -43 - index * 10) },
      ...(index < 2 ? [{ _id: id(`activity:${index}:change`), workspaceId: tenantId, projectId, actorId: approverId, actorName: "Jordan Lee", action: "change-request.approved", audience: "project", context: { actorRole: role, requestId: String(id(`change-request:${index}`)), requestNumber: 1, proposalId: String(id(`change-proposal:${index}`)), proposalNumber: 1, outcome: "approved" }, occurredAt: at(now, -43 - index * 10, 1) }] : []),
